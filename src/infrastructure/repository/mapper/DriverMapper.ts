...
export default class DriverMapper implements IMapper<DriverDTO, Driver> {
    map(input: DriverDTO): Driver {
        const { firstName, lastName, email, city, zipCode, street, status,
            timeActive, drivingLicense, registrationCertificate } = input;
        try {
            return new Driver({
                name: Name.create(NotEmptyString.create(firstName), NotEmptyString.create(lastName)),
                email: Email.create(email),
                address: Address.create(NotEmptyString.create(street), ZipCode.create(zipCode), NotEmptyString.create(city)),
